...
def get_folders_paths(embedding_file):
    ...
    main_folder_path = os.path.join(str(Path(__file__).parents[4]), 'lib')

    output_dir = os.path.join(main_folder_path, 'data/output')
    axilliary_dir = os.path.join(main_folder_path, 'data/axillary')
    quotes_dir = os.path.join(main_folder_path, 'data/quotes')

    embedding_file = os.path.join(axilliary_dir, args.embedding_file)
