...
from lib.helpers.utils import read_json, read_lines, remove_items_from_list_given_indices
...
parser.add_argument('--user_query', help='query of the user presented to the bot',
                    default='why should I trust a bot?')
...
def find_the_top_matched_quote(re, user_query_embedding, quotes_emb_dict, get_top_N_quotes, method):

    quotes = list(quotes_emb_dict.keys())

    sorted_quotes, unsorted_quotes = re.cosine_sim_query_quotes(user_query_embedding, quotes_emb_dict, method)
...
def run_method(args, method_name, output_dir, q_emb_file, user_query_emb, user_query):
        ...
        top_quotes, unsorted_quotes = find_the_top_matched_quote(re, user_query_emb, q_emb_dict, args.get_top_N_quotes, method_name)
...
def main_run(user_query):
    ...
    user_query_emb = gec.sentence_embedding(hmq_word_idf_dict, stop_words)
...
if __name__ == "__main__":
    ...
    formated_quotes_frame, formated_quotes_dict = main_run(args.user_query)

    print_out(args.user_query, formated_quotes_frame)
